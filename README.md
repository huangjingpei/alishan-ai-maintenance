...
7. [`docs/04_标识符可读性恢复工具链.md`](docs/04_标识符可读性恢复工具链.md)

## 标识符可读性恢复进度

业务代码中的 `_0x` 十六进制变量由恢复工具链分批清理（工具与验证流程见上文档）：

| 文件 | 状态 |
|---|---|
| `main/dbManager.js` | 已清理（2396 处，全部验证门通过） |
| `shared/commentAutomationController.js` | 已清理（10123 处 hex + 184 个函数语义命名） |

剩余文件按「改到哪、清到哪」推进；`npm run validate` 与 AST 等价校验是每批改动的硬性门禁。
